...
* `authentication_method` - (Required) user-authentication Uses the Azure AD User to authenticate. service-principal-authentication Uses the Service Principal to authenticate. managed-identity-authentication Uses the Managed Identity to authenticate. This option requires the Security Management Server be deployed in Azure and has a Managed Identity.
...
* `environment` - (Optional) Select the Azure Cloud Environment. Valid values are: AzureCloud, AzureChinaCloud, AzureUSGovernment.
