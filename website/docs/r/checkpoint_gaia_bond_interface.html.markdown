---
layout: "checkpoint"
page_title: "checkpoint_gaia_bond_interface"
sidebar_current: "docs-checkpoint-resource-checkpoint-gaia-bond-interface"
description: |-
This resource allows you to execute Check Point Bond Interface.
---

# checkpoint_gaia_bond_interface
...
```hcl
resource "checkpoint_gaia_bond_interface" "example" {
  resource_id = 5
  mode        = "8023AD"
  lacp_rate   = "slow"
}
...
* `xmit_hash_policy` - (Optional) layer2, layer3+4 
* `down_delay` - (Optional) Unit: Milliseconds 
* `up_delay` - (Optional) Unit: Milliseconds 
* `primary` - (Optional) Primary member of the bond interface 
* `lacp_rate` - (Optional)  
* `mode` - (Optional)  
* `mii_interval` - (Optional) Media monitoring interval 
* `min_links` - (Optional) Valid only while mode 8023AD is configured 
