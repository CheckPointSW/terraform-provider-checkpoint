...
func dataGaiaSetIpv6Pim() *schema.Resource {   
    return &schema.Resource{
        Create: createGaiaSetIpv6Pim,
        Read:   readGaiaSetIpv6Pim,
        Delete: deleteGaiaSetIpv6Pim,
...
func createGaiaSetIpv6Pim(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        success := err == nil && GaiaSetIpv6PimRes.Success
        errMsg := ""
        if err != nil {
            errMsg = err.Error()
        } else if !GaiaSetIpv6PimRes.Success {
            errMsg = GaiaSetIpv6PimRes.ErrorMsg
        }

        var respData map[string]interface{}
        if err == nil {
            respData = GaiaSetIpv6PimRes.GetData()
        }

        debugLogOperation(
            "set-ipv6-pim",        // resource type
            "command",                       // operation
            "set-ipv6-pim",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute set-ipv6-pim: %v", err)
    }
    if !GaiaSetIpv6PimRes.Success {
        if GaiaSetIpv6PimRes.ErrorMsg != "" {
            return fmt.Errorf(GaiaSetIpv6PimRes.ErrorMsg)
        }
        return fmt.Errorf("Unknown error occurred")
    }

    _respData := GaiaSetIpv6PimRes.GetData()
    if v, exists := _respData["custom-ssm-prefix"]; exists {
        if items, ok := v.([]interface{}); ok {
            ...
            d.Set("custom_ssm_prefix", out)
        }
    }
    if v, exists := _respData["spt-threshold"]; exists {
        if items, ok := v.([]interface{}); ok {
            out := make([]interface{}, 0, len(items))
            for _, item := range items {
                if m, ok := item.(map[string]interface{}); ok {
                    out = append(out, map[string]interface{}{
                        "multicast_group": fmt.Sprintf("%v", m["multicast-group"]),
                        "threshold":       fmt.Sprintf("%v", m["threshold"]),
                    })
                }
            }
            d.Set("spt_threshold", out)
        }
    }


    d.SetId(fmt.Sprintf("set-ipv6-pim-" + acctest.RandString(10)))
    return nil
}

func readGaiaSetIpv6Pim(d *schema.ResourceData, m interface{}) error {
    return nil
}

func deleteGaiaSetIpv6Pim(d *schema.ResourceData, m interface{}) error {
    d.SetId("")
    return nil
}
