...
func dataGaiaShowRoutes() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowRoutes,
...
func readGaiaShowRoutes(d *schema.ResourceData, m interface{}) error {
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-routes: %v", err)
    }
            ...
            for i, item := range raw {
                if m, ok := item.(map[string]interface{}); ok {
                    mapped[i] = map[string]interface{}{
                        ...
                        "tag": func() string { if _v, _ok := m["tag"]; _ok && _v != nil { return fmt.Sprintf("%v", _v) }; return "" }(),
    ...
    d.SetId(fmt.Sprintf("show-routes-" + acctest.RandString(10)))
    return nil
}
