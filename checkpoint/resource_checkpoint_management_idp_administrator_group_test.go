...
func testAccCheckpointManagementIdpAdministratorGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Primary.ID != "" {
			res, _ := client.ApiCall("show-idp-administrator-group", map[string]interface{}{"uid": rs.Primary.ID}, client.GetSessionID(), true, false)
			if res.Success {
...
func testAccCheckCheckpointManagementIdpAdministratorGroupExists(resourceTfName string, res *map[string]interface{}) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		response, err := client.ApiCall("show-idp-administrator-group", map[string]interface{}{"uid": rs.Primary.ID}, client.GetSessionID(), true, false)
		if !response.Success {
			return err
		}
