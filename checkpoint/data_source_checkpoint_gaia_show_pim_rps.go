...
func dataGaiaShowPimRps() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowPimRps,
...
func readGaiaShowPimRps(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "pim-rps",        // resource type
            "read",                       // operation
            "show-pim-rps",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-pim-rps: %v", err)
    }
    ...
    d.SetId(fmt.Sprintf("show-pim-rps-" + acctest.RandString(10)))
    return nil
}
