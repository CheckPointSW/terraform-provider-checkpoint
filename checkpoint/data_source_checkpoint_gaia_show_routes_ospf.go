...
func dataGaiaShowRoutesOspf() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowRoutesOspf,
...
func readGaiaShowRoutesOspf(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "routes-ospf",        // resource type
            "read",                       // operation
            "show-routes-ospf",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-routes-ospf: %v", err)
    }
    ...
    d.SetId(fmt.Sprintf("show-routes-ospf-" + acctest.RandString(10)))
    return nil
}
