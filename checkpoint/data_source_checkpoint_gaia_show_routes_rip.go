...
func dataGaiaShowRoutesRip() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowRoutesRip,
        Schema: map[string]*schema.Schema{
            ...
            "order": {
                ...
                Optional:    true,
                Description: `Sorts the routes by either ascending or descending order.`,
            },
            "address_family": {
                Type:        schema.TypeString,
                Optional:    true,
...
func readGaiaShowRoutesRip(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "routes-rip",        // resource type
            "read",                       // operation
            "show-routes-rip",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-routes-rip: %v", err)
    }
            ...
            for i, item := range raw {
                if m, ok := item.(map[string]interface{}); ok {
                    mapped[i] = map[string]interface{}{
                        ...
                        "tag": func() int { if f, ok := m["tag"].(float64); ok { return int(f) }; return 0 }(),
                    }
                }
            }
    ...
    d.SetId(fmt.Sprintf("show-routes-rip-" + acctest.RandString(10)))
    return nil
}
