...
func dataGaiaShowRoutesAggregate() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowRoutesAggregate,
...
func readGaiaShowRoutesAggregate(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "routes-aggregate",        // resource type
            "read",                       // operation
            "show-routes-aggregate",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-routes-aggregate: %v", err)
    }
    ...
    d.SetId(fmt.Sprintf("show-routes-aggregate-" + acctest.RandString(10)))
    return nil
}
