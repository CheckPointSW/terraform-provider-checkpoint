...
func dataGaiaShowBgpRoutesOut() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowBgpRoutesOut,
...
func readGaiaShowBgpRoutesOut(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "bgp-routes-out",        // resource type
            "read",                       // operation
            "show-bgp-routes-out",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-bgp-routes-out: %v", err)
    }
    ...
    d.SetId(fmt.Sprintf("show-bgp-routes-out-" + acctest.RandString(10)))
    return nil
}
