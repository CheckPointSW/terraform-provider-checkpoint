...
func dataGaiaShowRoutesBgp() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowRoutesBgp,
...
func readGaiaShowRoutesBgp(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "routes-bgp",        // resource type
            "read",                       // operation
            "show-routes-bgp",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-routes-bgp: %v", err)
    }
    ...
    d.SetId(fmt.Sprintf("show-routes-bgp-" + acctest.RandString(10)))
    return nil
}
