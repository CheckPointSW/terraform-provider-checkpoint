...
func dataGaiaShowBgpRouteIn() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowBgpRouteIn,
...
func readGaiaShowBgpRouteIn(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "bgp-route-in",        // resource type
            "read",                       // operation
            "show-bgp-route-in",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-bgp-route-in: %v", err)
    }
    ...
    d.SetId(fmt.Sprintf("show-bgp-route-in-" + acctest.RandString(10)))
    return nil
}
