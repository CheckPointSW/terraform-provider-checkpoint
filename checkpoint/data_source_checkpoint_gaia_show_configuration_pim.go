...
func dataGaiaShowConfigurationPim() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowConfigurationPim,
...
func readGaiaShowConfigurationPim(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "configuration-pim",        // resource type
            "read",                       // operation
            "show-configuration-pim",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-configuration-pim: %v", err)
    }
    ...
    d.SetId(fmt.Sprintf("show-configuration-pim-" + acctest.RandString(10)))
    return nil
}
