...
func dataGaiaShowPimJoins() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowPimJoins,
...
func readGaiaShowPimJoins(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "pim-joins",        // resource type
            "read",                       // operation
            "show-pim-joins",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-pim-joins: %v", err)
    }
    ...
    d.SetId(fmt.Sprintf("show-pim-joins-" + acctest.RandString(10)))
    return nil
}
