...
func dataGaiaShowStaticRoutes() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowStaticRoutes,
...
func readGaiaShowStaticRoutes(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "static-routes",        // resource type
            "read",                       // operation
            "show-static-routes",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-static-routes: %v", err)
    }
            ...
            for i, item := range raw {
                if m, ok := item.(map[string]interface{}); ok {
                    mapped[i] = map[string]interface{}{
                        ...
                        "scope_local": func() bool { if b, ok := m["scope-local"].(bool); ok { return b }; if s, ok := m["scope-local"].(string); ok { return s == "true" }; return false }(),
    ...
    d.SetId(fmt.Sprintf("show-static-routes-" + acctest.RandString(10)))
    return nil
}
