...
func dataGaiaShowNatPools() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowNatPools,
...
func readGaiaShowNatPools(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "nat-pools",        // resource type
            "read",                       // operation
            "show-nat-pools",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-nat-pools: %v", err)
    }
            ...
            for i, item := range raw {
                if m, ok := item.(map[string]interface{}); ok {
                    mapped[i] = map[string]interface{}{
                        "prefix": func() string { if _v, _ok := m["prefix"]; _ok && _v != nil { return fmt.Sprintf("%v", _v) }; return "" }(),
    ...
    d.SetId(fmt.Sprintf("show-nat-pools-" + acctest.RandString(10)))
    return nil
}
