...
func dataGaiaShowConfigurationBgpConfederationPeer() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowConfigurationBgpConfederationPeer,
...
func readGaiaShowConfigurationBgpConfederationPeer(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "configuration-bgp-confederation-peer",        // resource type
            "read",                       // operation
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-configuration-bgp-confederation-peer: %v", err)
    }
    ...
    d.SetId(fmt.Sprintf("show-configuration-bgp-confederation-peer-" + acctest.RandString(10)))
    return nil
}
