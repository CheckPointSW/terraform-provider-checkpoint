...
func testAccCheckpointManagementExceptionGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Primary.ID != "" {
			res, _ := client.ApiCall("show-exception-group", map[string]interface{}{"uid": rs.Primary.ID}, client.GetSessionID(), true, client.IsProxyUsed())
			if res.Success {
...
func testAccCheckCheckpointManagementExceptionGroupExists(resourceTfName string, res *map[string]interface{}) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		response, err := client.ApiCall("show-exception-group", map[string]interface{}{"uid": rs.Primary.ID}, client.GetSessionID(), true, client.IsProxyUsed())
		if !response.Success {
			return err
		}
