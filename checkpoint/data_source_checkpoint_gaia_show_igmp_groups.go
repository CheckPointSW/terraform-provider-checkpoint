...
func dataGaiaShowIgmpGroups() *schema.Resource {   
    return &schema.Resource{
        Read:   readGaiaShowIgmpGroups,
        Schema: map[string]*schema.Schema{
            ...
            "order": {
                ...
                Optional:    true,
                Description: `Sorts the interface IGMP groups entries by interface name in either ascending or descending order`,
            },
            "type": {
                Type:        schema.TypeString,
                Optional:    true,
                Description: `The type of IGMP group`,
            },
            "interface": {
                Type:        schema.TypeString,
                Optional:    true,
                Description: `The name of the interface associated with the IGMP groups`,
            },
            "member_id": {
                Type:        schema.TypeString,
                Optional:    true,
...
func readGaiaShowIgmpGroups(d *schema.ResourceData, m interface{}) error {
    ...
    if resourceDebugEnabled(d) {
        ...
        debugLogOperation(
            "igmp-groups",        // resource type
            "read",                       // operation
            "show-igmp-groups",         // API call name
    ...
    if err != nil {
        return fmt.Errorf("Failed to execute show-igmp-groups: %v", err)
    }
            ...
            for i, item := range raw {
                if m, ok := item.(map[string]interface{}); ok {
                    mapped[i] = map[string]interface{}{
                        ...
                        "expire": func() int { if f, ok := m["expire"].(float64); ok { return int(f) }; return 0 }(),
                    }
                }
            }
    ...
    d.SetId(fmt.Sprintf("show-igmp-groups-" + acctest.RandString(10)))
    return nil
}
